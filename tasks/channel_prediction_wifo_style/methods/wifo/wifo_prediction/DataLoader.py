...
def _dataset_root() -> Path:
    default_root = (
        ...
        / "datasets"
        / "wifo_channel_prediction"
        / "data"
        / "test_data"
    )
    return Path(os.path.expandvars(os.environ.get("WIFO_DATASET_ROOT", str(default_root)))).expanduser()


def data_load_single(args, dataset):  # 加载单个数据集
    folder_path_test = _dataset_root() / dataset / "X_test.mat"
