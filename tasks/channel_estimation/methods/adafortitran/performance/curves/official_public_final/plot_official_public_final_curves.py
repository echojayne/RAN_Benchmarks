"""Plot official-public AdaFortiTran evaluation curves."""
...
RAW_CSV = (
    CURVE_DIR.parents[1]
    / "raw_data"
    / "official_public_final_eval"
    / "official_public_final_metrics.csv"
)
...
AXES = {
    "snr": {
        ...
        "xlabel": "SNR (dB)",
        "output": "official_public_final_mse_db_vs_snr.png",
    },
    "delay_spread": {
        "title": "Delay Spread Robustness",
        "xlabel": "Delay spread (ns)",
        "output": "official_public_final_mse_db_vs_delay_spread.png",
    },
    "doppler": {
        "title": "Doppler Robustness",
        "xlabel": "Doppler (Hz)",
        "output": "official_public_final_mse_db_vs_doppler.png",
    },
}
...
def plot_single_axis(rows: list[dict[str, str]], axis: str) -> Path:
    ...
    ax.plot(xs, ys, marker="o", linewidth=2.0, color="#1f6f8b")
...
def plot_combined(rows: list[dict[str, str]]) -> Path:
    ...
    for ax, axis in zip(axes, AXES, strict=True):
        ...
        ax.grid(True, alpha=0.3)
    fig.tight_layout()
    output = CURVE_DIR / "official_public_final_all_curves.png"
    fig.savefig(output, dpi=220)
    plt.close(fig)
    return output
